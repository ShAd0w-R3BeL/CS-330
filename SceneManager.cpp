...
void SceneManager::LoadSceneTextures()
{
	...
	CreateGLTexture("../../Utilities/textures/rusticwood.jpg", "woodTexture");

	// Load the brick texture for the cone
	CreateGLTexture("../../Utilities/textures/textures/sun.png", "sunTexture");
	...
	CreateGLTexture("../../Utilities/textures/grey.jpeg", "greyTexture");

	// Load the red texture for the box
	CreateGLTexture("../../Utilities/textures/white.jpeg", "whiteTexture");

	CreateGLTexture("../../Utilities/textures/black.png", "blackTexture");
...
void SceneManager::RenderScene() {
	...
	positionXYZ = glm::vec3(6.5f, 5.0f, -1.0f);
	...
	positionXYZ = glm::vec3(-2.3f, 1.0f, 5.0f);
	...
	positionXYZ = glm::vec3(5.0f, 0.4f, 3.0f);
	SetTransformations(scaleXYZ, 0.0f, 0.0f, 0.0f, positionXYZ);
	SetShaderTexture("blackTexture");
	m_basicMeshes->DrawCylinderMesh();
	...
	positionXYZ = glm::vec3(5.0f, 0.5f, 2.6f);
	SetTransformations(scaleXYZ, 0.0f, 90.0f, 0.0f, positionXYZ);
	SetShaderColor(0.5f, 0.5f, 0.5f, 1.0f); // Darker grey
	m_basicMeshes->DrawTorusMesh();
	...
	float keyOffsetX = -3.5f;   // Start position of the first key on the X-axis
	float keyOffsetZ = 0.5f;   // Start position of the first key on the Z-axis

	for (int i = 0; i < 4; i++) {  // 4 rows of keys
		for (int j = 0; j < 15; j++) {  // 10 keys per row
			scaleXYZ = glm::vec3(0.25f, 0.05f, 0.15f);  // Keep keys relatively the same size
			positionXYZ = glm::vec3(keyOffsetX + (j * keySpacingX), 0.16f, keyOffsetZ + (i * keySpacingZ));  // Adjust key position
			SetTransformations(scaleXYZ, 0.0f, 0.0f, 0.0f, positionXYZ);
